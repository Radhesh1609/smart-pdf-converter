...
export default function PngToPdf({ onBack }: PngToPdfProps) {
  ...
  const handleFileSelect = (files: FileList | null) => {
    ...
    const newImages = Array.from(files).filter((file) => file.type === "image/png")
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto">
      ...
      <div className="backdrop-blur-xl bg-slate-800/30 border border-slate-700/50 rounded-2xl p-8">
        <h2 className="text-3xl font-bold mb-2 bg-gradient-to-r from-green-400 to-emerald-500 bg-clip-text text-transparent">
          PNG to PDF Converter
        </h2>
        <p className="text-slate-400 mb-8">Convert your PNG images to PDF format</p>
        ...
        <div
          ...
          className="border-2 border-dashed border-slate-600/50 rounded-xl p-12 text-center cursor-pointer hover:border-green-500/50 hover:bg-green-500/5 transition-all duration-300 mb-8"
        >
          <input
            ref={fileInputRef}
            type="file"
          ...
          <p className="text-lg font-semibold mb-2">Drag & drop PNG files here</p>
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 rounded-lg bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 text-white font-semibold"
          >
            {isConverting ? (
              <>
